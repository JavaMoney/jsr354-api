 ...
 * scientific notation. Example: <code>"0.###E0"</code> formats the number 1234
 * as <code>"1.234E3"</code>.
 * 
 * <ul>
 ...
 * @author Alan Liu
 */
public abstract class DecimalFormat extends NumberFormat { // TODO abstract
															// temporary for
															// stubbing
	...
	/*
	 * Detecting whether a double is negative is easy with the exception of the
	 * value -0.0. This is a double which has a zero mantissa (and exponent),
	 * but a negative sign bit. It is semantically distinct from a zero with a
	 * positive sign bit, and this distinction is important to certain kinds of
	 * computations. However, it's a little tricky to detect, since (-0.0 ==
	 * 0.0) and !(-0.0 < 0.0). How then, you may ask, does it behave distinctly
	 * from +0.0? Well, 1/(-0.0) == -Infinity. Proper detection of -0.0 is
	 * needed to deal with the issues raised by bugs 4106658, 4106667, and
	 * 4147706. Liu 7/6/98.
	 */
	...
	// /**
	// * Complete the formatting of a finite number. On entry, the digitList
	// must
	// * be filled in with the correct digits.
	// */
	// private StringBuffer subformat(StringBuffer result, FieldDelegate
	// delegate,
	...
	// backup = position;
	// } else if (!isExponent && text.regionMatches(position, exponentString, 0,
	 ...
	 * @see java.text.DecimalFormatSymbols#getGroupingSeparator
	 */
	public byte[] getGroupingSizes() {
		return groupingSizes.clone();
	}
	...
	public void setGroupingSize(int newValue) {
		if (newValue < 0) {
			throw new IllegalArgumentException("groupingSize >= 0.");
		}
		groupingSizes = new byte[] { (byte) newValue };
	}
	
	/**
	 * Set the grouping sizes. Grouping sizes is the number of digits between
	 * grouping separators in the integer portion of a number, starting from the 
	 * decimal separator. For example, in the number "10,33,23,456.78", the 
	 * grouping size is {3, 2}. <br>
	 * The value passed in is converted to a byte, which may lose information.
	 * 
	 * @see #getGroupingSizes
	 * @see java.text.NumberFormat#setGroupingUsed
	 * @see java.text.DecimalFormatSymbols#setGroupingSeparator
	 */
	public void setGroupingSizes(byte... newValues) {
		if (newValues == null || newValues.length==0) {
			throw new IllegalArgumentException("groupingSizes == null, groupingSizes.length > 0.");
		}
	...
	public boolean equals(Object obj) {
		...
		return ((posPrefixPattern == other.posPrefixPattern && positivePrefix
				...
				&& Arrays.equals(groupingSizes, other.groupingSizes)
	...
	private void appendAffix(StringBuffer buffer, String affix,
		...
		if (localized) {
			needQuote = affix.indexOf(symbols.getZeroDigit()) >= 0
					|| affix.indexOf(symbols.getGroupingSeparator()[0]) >= 0  // TODO fix implementation here
	...
	private String toPattern(boolean localized) {
		...
		for (int j = 1; j >= 0; --j) {
			...
			int digitCount = useExponentialNotation ? getMaximumIntegerDigits()
					: Math.max(groupingSizes[0], getMinimumIntegerDigits()) + 1; // TODO adapt grouping sizes
			for (i = digitCount; i > 0; --i) {
				if (i != digitCount && isGroupingUsed() && groupingSizes[0] != 0 // TODO adapt grouping sizes
						&& i % groupingSizes[0] == 0) {                          // TODO adapt grouping sizes
	...
	private void applyPattern(String pattern, boolean localized) {
		...
		if (localized) {
			...
			groupingSeparator = symbols.getGroupingSeparator()[0];   // TODO adapt grouping separators
		...
		for (int j = 1; j >= 0 && start < pattern.length(); --j) {
			...
			boolean groupingEvaluated = false;
			...
			for (int pos = start; pos < pattern.length(); ++pos) {
				...
				case 1:
					...
					if (ch == digit) {
						...
						if (symbols.isInverseGrouping()) {
							groupingEvaluated = true;
						} else {
							groupingCount = 0;
						}
			...
			if (j == 1) {
				...
				this.groupingSizes = (groupingCount > 0) ? new byte[]{groupingCount} : new byte[]{0}; // TODO add support for multiple groups
	...
	public void setMaximumFractionDigits(int newValue) {
		...
		super.setMaximumFractionDigits((maximumFractionDigits > DOUBLE_FRACTION_DIGITS) ? DOUBLE_FRACTION_DIGITS
				: maximumFractionDigits);
		if (minimumFractionDigits > maximumFractionDigits) {
			minimumFractionDigits = maximumFractionDigits;
			super.setMinimumFractionDigits((minimumFractionDigits > DOUBLE_FRACTION_DIGITS) ? DOUBLE_FRACTION_DIGITS
	...
	void adjustForCurrencyDefaultFractionDigits() {
		...
		if (currency == null) {
			try {
				currency = Currency.getInstance(symbols
	 ...
	 * <ol>
	 * <li>
	 * Verify that the superclass's digit count fields correctly reflect the
	 ...
	 * <li>
	 * If <code>serialVersionOnStream</code> is less than 3, then call the
	 ...
	 * <li>
	 * If <code>serialVersionOnStream</code> is less than 1, indicating that the
	 ...
	 * of a number. Each item must be greater than 0 if
	 * <code>NumberFormat.groupingUsed</code> is true. A size of 0 equals to
	 * setting groupingUsed to false.
	 * 
	 * @serial
	 * @see #getGroupingSize
	 * @see java.text.NumberFormat#isGroupingUsed
	 */
	private byte[] groupingSizes = new byte[] { 3 }; // invariant, > 0 if
														// useThousands
	 ...
	 */
	private static final ConcurrentMap<Locale, String> cachedLocaleData = new ConcurrentHashMap<Locale, String>(
			3);
}
