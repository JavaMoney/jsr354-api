 ...
 *
 *
 * TODO Package should be java.util, this is a stub for the equivalent OpenJDK class
 */
 ...
 * codes. Visit the <a
 * href="http://www.iso.org/iso/en/prods-services/popstds/currencycodes.html">
 * ISO web site</a> for more information, including a table of currency codes.
 * <p>
 ...
 * file named <code>&lt;JAVA_HOME&gt;/lib/currency.properties</code>. The
 * contents of the properties file are key/value pairs of the ISO 3166 country
 * codes and the ISO 4217 currency data respectively. The value part consists of
...
public final class Currency implements LocalizableCurrencyUnit, Serializable {
	...
	private static HashMap<String, Currency> instances = new HashMap<String, Currency>(
			7);
	...
	static {
		AccessController.doPrivileged(new PrivilegedAction<Object>() {
			public Object run() {
				...
				try {
					File propFile = new File(homeDir + File.separator + "lib"
	...
	private static Currency getInstance(String currencyCode,
		...
		synchronized (instances) {
			...
			// Also, if there already is an entry, the currencyCode must be
			// valid.
			...
			if (defaultFractionDigits == Integer.MIN_VALUE) {
				...
				if ((tableEntry & COUNTRY_TYPE_MASK) == SIMPLE_CASE_COUNTRY_MASK
					...
					// Check for '-' separately so we don't get false hits in
					// the table.
	 ...
	 *         locale, or null
	 * @exception NullPointerException
	 *                if <code>locale</code> or its country code is null
	 * @exception IllegalArgumentException
	...
	public String getSymbol(Locale locale) {
		try {
			// Check whether a provider can provide an implementation that's
			// closer
			...
			if (pool.hasProviders()) {
				// Assuming that all the country locales include necessary
				// currency
				// symbols in the Java runtime's resources, so there is no need
				// to
				// examine whether Java runtime's currency resource bundle is
				// missing
				// names. Therefore, no resource bundle is provided for calling
				// this
				// method.
				String symbol = pool.getLocalizedObject(
						CurrencyNameGetter.INSTANCE, locale,
						(OpenListResourceBundle) null, currencyCode, SYMBOL);
				if (symbol != null) {
					return symbol;
				}
			}
	...
	public String getDisplayName(Locale locale) {
		try {
			...
			LocaleServiceProviderPool pool = LocaleServiceProviderPool
					.getPool(CurrencyNameProvider.class);
			if (pool.hasProviders()) {
				result = pool.getLocalizedObject(CurrencyNameGetter.INSTANCE,
	 ...
	 *            and "one-digit (0,1,2, or 3) default fraction digit". For
	 *            example, "JPZ,392,0".
	 * @throws
	 */
	private static void replaceCurrencyData(Pattern pattern, String ctry,
		...
		if (ctry.length() != 2) {
			...
			String message = new StringBuilder()
					...
					.append(ctry)
					.append(" is ignored because of the invalid country code.")
					.toString();
			info(message, null);
			return;
		}
