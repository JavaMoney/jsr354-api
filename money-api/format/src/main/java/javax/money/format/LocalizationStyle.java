...
public final class LocalizationStyle implements Serializable {
     ...
     */
    private LocalizationStyle(Class<?> targetType, String id, Map<String, Object> attributes) {
	this.id = id;
	...
	public LocalizationStyle build(boolean register) {
	    if (!isBuildable()) {
		throw new IllegalStateException("Builder can not build a LocalizationStyle.");
	    }
	    LocalizationStyle style = new LocalizationStyle(this.targetType, this.id, this.attributes);
	    if (register) {
