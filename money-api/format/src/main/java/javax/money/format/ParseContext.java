...
public final class ParseContext<T> {
	...
	public static interface ItemFactory<T> extends
			MonetaryFunction<ParseContext<T>, T> {
		/**
		 * Returns true, if the required item is available from the parsed
		 * results.
		 * 
		 * @param context
	...
	public T getItem() {
		...
		T item = this.itemFactory.apply(this);
		if (item == null) {
