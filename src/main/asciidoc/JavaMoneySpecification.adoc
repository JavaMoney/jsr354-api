...
the JSR itself, enabling feature innovation elsewhere. The corresponding libraries were published under [JavaMoney]
...
jsr354-api:: contains the JSR 354 API as described also by this specification.
jsr354-ri:: contains the ''moneta'' reference implementation.
jsr354-tck:: contains the technical compatibility kit (TCK).
...
It is also planned to provide a Java 7 based backport of the API and RI.
...
public interface MonetaryFormatsSingletonSpi{
  ...
  default MonetaryAmountFormat getAmountFormat(Locale locale, String... providers){...}    <1>
  default MonetaryAmountFormat getAmountFormat(String name, String... providers);          <1>
  Collection<MonetaryAmountFormat> getAmountFormats(AmountFormatQuery query);              <2>
  default boolean isAvailable(Locale locale, String... providers){...}                     <3>
  default boolean isAvailable(AmountFormatQuery formatQuery){...}                          <4>
  Set<Locale> getAvailableLocales(String... providers);                                    <5>
  Set<String> getFormatNames(String... providers);                                         <6>
  Set<String> getProviderNames();                                                          <7>
  List<String> geDefaultProviderChain();                                                   <8>
}
-------------------------------------------------------------------------------
<1> By default all this methods are mapped into a corresponding instance of +AmountFormatQuery+ and passed to <2>.
<2> Main method called for accessing formats.
<3> This method is mapped into a corresponding instance of +AmountFormatQuery+ and passed to <4>.
<4> This method by default tries to access a formatting calling <2>.
<5> Collect all locales available from the given providers. If no provider IDs are passed the default provider chain
    is used.
<6> Get the available format names for the given providers. If no provider IDs are passed the default provider chain
    is used.
<7> Access all registered provider's ids.
<8> Access the ids of the default provider chain, in the ordering as executing.
