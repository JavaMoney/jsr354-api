...
which is capable of providing arbitrary meta-data on the current instance.
...
* the +CurrencyContext+ models additional metadata of a currency unit (refer to section <<metadata modelling>> for more
  details on contexts). It basically allows to evaluate the data
  provider of a currency unit, but can also contain any arbitrary additional data, determined by the implementation
  that provided the currency instance. This context allows to support also more complex use cases for extended
  currency meta-data such as:
** validity timestamps
...
NOTE: The operator interface is equivalent to the +UnaryOperator+ interface in JDK 8 which is a functional interface
...
The monetary context models the amount meta-data, including the numeric capabilities of a monetary amount
(implementation) in a platform independent way (refer also to section <<MetadataModeling>> for more
...
For further details on context and meta-data modeling, refer to section <<MetadataModeling>>.
...
addition allows registration of additional currencies by implementing an instances
...
Collection<CurrencyUnit> currencies = MonetaryCurrencies.getCurrencies(
  CurrencyQuery.create().setProvider("A", "B").set(Region.EMEA).setInt("contractNr", 12345).build()
);
-------------------------------------------------------------------------------

The query interface also is flexible enough to support access to historic currencies. As an example, if an according
provider would implemented and registered one could perform the following query:
...
Collection<CurrencyUnit> currencies = MonetaryCurrencies.getCurrencies(
  CurrencyQuery.create().set(Locale.GERMANY).setTimestamp(LocalDate.of(1930,1,1)).build()
);
-------------------------------------------------------------------------------
...
. an explicit (unique) _rounding id_ that must be known (and documemted) by a +RoundingProviderSpi+ implementation.
...
implements Serializable{
  ...
  public long getInt(Object key, int defaultValue);
...
The classes extending this class hereby are thread-safe and immutable, since context instances must be created by
a corresponding builder implementation.
...
implements Serializable{
  ...
  public B set(Object value, Class<?> type);
  ...
  public B set(Set<?> set);
  public B set(Object key, Set<?> set);
  public B set(List<?> set);
  public B set(Object key, List<?> set);
...
[RED]
TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED
TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED
TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED
TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED
TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED TO BE CONTINUED
