...
:revnumber: 1.1
:revremark: Maintenance Release 1
:revdate: May 2020
:longversion: 1.1 (MR 1) May 2020
...
As it is the first Java LTS version and applications in the financial area will probably use Java 8 for several years this JSR supports
Java 8 as the minimum version.
...
and comprehensive API that is compatible with Java 8, and also with Java SE Embedded.
...
The specification supports Java SE platforms version 8 and above.
The API is backward compatible with Java 8 while exposing a JPMS (Jigsaw) module for Java 9 and above, 
so implementations may target any suitable Java SE version.
The reference implementation "Moneta" is modular and supports the Java Platform Module System (JPMS) as well as OSGi bundles, but it's binary compatible with Java 8 as the minimal Java version.
...
-> Building blocks should be modeled/organized in a common repository and accessible by a common API, that also allows
...
Examples like <<JadiraFramework>>, <<JakartaBeanValidation>>, <<SpringSupport>> or <<ZalandoJacksonBinding>> show a broad support for JSR 354 in the Java EE, Jakarta EE or Spring ecosystems.
...
. Queries and contexts may require adding additional time related data, such as POSIX timestamps based on millisecond
  ...
  These aspects are not explicitly modeled, since they depend on the capabilities of the corresponding providers
...
jsr354-tck:: contains the technical compatibility kit (TCK). The TCK is built using Java 8.
...
So the +CurrencyUnit+ interface for currencies is modeled only with 4 methods as follows:
...
** validity range, e.g. modeled as from/to +LocalDate+
...
effective data and functional interoperability. In general the following aspects are modeled:
...
* +getCurrency()+ returns the amount’s currency, modeled as +CurrencyUnit+. Implementations may co-variantly change the
...
operations, being applied on amounts. This is modeled by
...
Functional interfaces are annotated with the +@FunctionalInterface+ annotation.
...
** basically additional aspects can be modeled as useful, but are not defined by this specification, e.g. the
...
As illustrated above the signature of +MonetaryAmountFactory+ is modeled as a builder also supporting a fluent
programming style:
...
* access to providers that offer conversion rates, modeled as +<<ExchangeRateProviders, ExchangeRate>>+.
...
==== Converting Amounts
Basically converting of amounts is modeled by the +CurrencyConversion+ interface which extends +MonetaryOperator+,
hereby adding meta-data support, modeled by +ConversionContext+. Hereby a *conversion is always bound to a specific
...
Summarizing an +ExchangeRate+ is modeled as follows:
...
Summarizing an +ExchangeRateProvider+ is modeled as follows:
...
public interface ExchangeRateProvider{
  ...
  // modeled as default methods in Java 8
...
The formatting is modeled with a quite simple, but very flexible design. It allows the access of formats based
...
public interface MonetaryFormatsSingletonSpi{
  ...
  // The following methods are modeled as default methods in Java 8
...
* Finally there is always the possibility that no common ground can be found for the way some functionality can be
  modeled generically across implementations. It would then be the responsibility of the implementers to follow best,
