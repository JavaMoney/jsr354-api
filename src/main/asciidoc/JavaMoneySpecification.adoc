...
So the interface for currencies is modelled only with 4 methods as follows:
...
* _Data interoperability_ allowing access to the amount’s
   ...
   ** Methods for evaluating the _numeric capabilities_ of the concrete type and further meta-data (+MonetaryContext+).
...
* rounding of amounts, see <<MonetaryRounding>>
...
(implementation) in a platform independent way (refer also to section <<metadata modelling>> for more
...
For further details on context and meta-data modeling, refer to section <<metadata modelling>>.
