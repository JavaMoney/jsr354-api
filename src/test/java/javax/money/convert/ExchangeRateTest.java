...
public class ExchangeRateTest {
	...
	public void testExchangeRateExchangeRateTypeCurrencyUnitCurrencyUnitNumberString() {
		...
		ExchangeRate rate = new DefaultExchangeRate.Builder("myProvider",
				RateType.DEFERRED).setBase(base).setTerm(term).setFactor(TestNumberValue.of(1.5))
				.build();
	...
	public void testExchangeRateExchangeRateTypeCurrencyUnitCurrencyUnitNumberStringLongLong() {
		CurrencyUnit base = TestCurrency.of("CHF");
		CurrencyUnit term = TestCurrency.of("EUR");
		ExchangeRate rate = new DefaultExchangeRate.Builder("test", RateType.DEFERRED)
				.setBase(base).setTerm(term).setFactor(TestNumberValue.of(1.5)).build();
		assertEquals(base, rate.getBase());
		assertEquals(term, rate.getTerm());
		assertTrue(1.5d == rate.getFactor().doubleValue());
	...
	public void testExchangeRateExchangeRateTypeCurrencyUnitCurrencyUnitNumberStringExchangeRateArray() {
		...
		ExchangeRate rate1 = new DefaultExchangeRate.Builder("test", RateType.DEFERRED)
				.setBase(base).setTerm(baseTerm).setFactor(TestNumberValue.of(0.8)).build();
		ExchangeRate rate2 = new DefaultExchangeRate.Builder("test", RateType.DEFERRED)
				.setBase(baseTerm).setTerm(term).setFactor(TestNumberValue.of(1.4)).build();

		// derived rate
		ExchangeRate rate = new DefaultExchangeRate.Builder("test", RateType.DEFERRED)
				.setBase(base).setTerm(term).setFactor(TestNumberValue.of(0.8 * 1.4))
				.setRateChain(rate1, rate2).build();
